...
AudioConnection          patchCord22(i2s1,0, queue1,0);
...
void sendFloat(float value) {
  Serial3.write(0x02); // Start marker
  Serial3.write((byte*)&value, sizeof(value));
  Serial3.write(0x03); // End marker
}
...
arduinoMFCC mfcc(numChannels, frameSize, hopSize, mfccSize, sampleRate);

// Memory Allocation

float *mfcc_coeffs   =(float *)malloc(numChannels * sizeof(float ));
float *rmfcc_coeffs  =(float *)malloc(mfccSize * sizeof(float ));
float **mfcc_array; // 2D array to store MFCC coefficients for each frame
float **mel_filter   = (float **)malloc(numChannels * sizeof(float *));
...
int frameCount = 0;
const int totalFrames = 225; // Stop after 5.22secs
...
void MFCCComputationTask(void *pvParameters) {
  Serial.begin(115200);
  Serial3.begin(115200); // Initialize UART for communication with ESP32
  ...
  while (1) {
        if (queue.available() >=8  && frameCount < totalFrames) {
            ...
            frameCount++;
            if (frameCount >= totalFrames && firstperioddone==false ) {
                firstperioddone =true;
                frameCount=0;
                clearMfccArray(); 
            }
            
            if (frameCount >= totalFrames && firstperioddone== true){
                // Transmit the MFCC array via UART to LPSTK
                Serial3.write(0x01); // Start byte
                Serial3.write((uint8_t)(totalFrames >> 8)); // Send the high byte first
                Serial3.write((uint8_t)totalFrames); // Send the low byte
                Serial3.write((uint8_t)mfccSize); // Number of coefficients per frame

                for (int i = 0; i < totalFrames; i++) {
                    uint8_t checksum = calculateChecksum(mfcc_array[i], mfccSize);
                    Serial3.write((byte *)&checksum, sizeof(checksum)); // Send checksum
                    for (int j = 0; j < mfccSize; j++) {
                        Serial3.write((byte *)&mfcc_array[i][j], sizeof(mfcc_array[i][j])); // Send each MFCC coefficient as bytes
                        vTaskDelay(1 /portTICK_PERIOD_MS);
                    }
                }
                Serial3.write(0x04); // End byte    
...
void setup() {
    ...
    //xTaskCreate(LAeqComputationTask, "LAeqTask", 8192, NULL, 1, &xLAeqComputationTask);
    //xTaskCreate(AudioRecordingTask, "AudioRecTask", 8192, NULL, 1, &xAudioRecordingTask);
    xTaskCreate(MFCCComputationTask, "MFCCComTask",8192, NULL, 1, &xMFCCComputationTask);
