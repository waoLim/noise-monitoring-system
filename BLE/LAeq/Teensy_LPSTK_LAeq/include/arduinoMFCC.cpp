...
arduinoMFCC::arduinoMFCC(uint8_t  num_channels, uint16_t  frame_size, uint8_t  hop_size, uint8_t  mfcc_size, float samplerate) {
    ...
    // Serial.println("arduinoMFCC constructor called");	
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
arduinoMFCC::~arduinoMFCC() {
    ...
    // Serial.println("arduinoMFCC destructor called");
}
/////////////////////////////////////////////////////////////////////////////
void arduinoMFCC::pre_emphasis(uint16_t _frame_size, float *_frame)
{
// pre-emphasis
    // Serial.println("Applying pre-emphasis");
...
void arduinoMFCC::computebust_dct(uint8_t _mfcc_size,uint8_t _num_channels,uint16_t  _frame_size,float *_rmfcc_coeffs) {
    ...
    apply_hamming_window(_frame,_hamming_window);
    apply_mel_filter_bank(_num_channels,_frame_size,_frame,_mel_filter_bank,_mfcc_coeffs);
    apply_dct(_mfcc_size,_num_channels,_frame_size,_mel_filter_bank,_mfcc_coeffs,_rmfcc_coeffs,_dct_matrix);
    }
...
void arduinoMFCC::create_hamming_window(uint16_t  _frame_size, float *_hamming_window) {
    // Serial.println("Creating Hamming window");
    // ... code pour créer la fenêtre de Hamming ...
	// Fonction publique pour créer la fenêtre de Hamming
    for (uint16_t  i = 0; i < _frame_size; i++) {
        _hamming_window[i] = 0.54 - 0.46 * cos(2 * PI * i / (_frame_size - 1));
...
{
    // Allocate memory for the array of pointers to float arrays
    _mel_filter_bank = (float**)malloc(_num_channels * sizeof(float*));
    if (_mel_filter_bank == NULL) {
    ...
    for (uint8_t i = 0; i < _num_channels; i++) {
        _mel_filter_bank[i] = (float*)malloc((_frame_size / 2) * sizeof(float));
        if (_mel_filter_bank[i] == NULL) {
            // Memory allocation failed
            // Handle the error appropriately, e.g., free previously allocated memory and return
            for (uint8_t j = 0; j < i; j++) {
                free(_mel_filter_bank[j]);
            }
            free(_mel_filter_bank);
            return;
        }
    ...
    // Calculations for Mel and Hz scale points
    float f_low          = 300.0;  
    float f_high         = _samplerate; // Nyquist Frequency
    float mel_low_freq   = 2595.0 * log10f(1 + (f_low / 2.0) / 700.0);
    float mel_high_freq  = 2595.0 * log10f(1 + (f_high / 2.0) / 700.0);
    float* mel_f         = (float*)malloc((_num_channels + 2) * sizeof(float));
    float* hzPoints      = (float*)malloc((_num_channels + 2) * sizeof(float));
    if (mel_f == NULL || hzPoints == NULL) {
        // Memory allocation failed
        // Handle the error appropriately, e.g., free previously allocated memory and return
        for (uint8_t i = 0; i < _num_channels; i++) {
            free(_mel_filter_bank[i]);
        ...
        free(_mel_filter_bank);
        if (mel_f != NULL) free(mel_f);
        if (hzPoints != NULL) free(hzPoints);
        return;
    }
    ...
    // Free dynamically allocated memory for temporary arrays
    free(mel_f);
    free(hzPoints);
}
//////////////////////////////////////////////////////////////////////////////////
...
void arduinoMFCC::create_dct_matrix(float **&_dct_matrix) {
    // Allocate memory for the array of pointers to float arrays
    _dct_matrix = (float**)malloc(_mfcc_size * sizeof(float*));
    if (_dct_matrix == NULL) {
        // Memory allocation failed
        // Handle the error appropriately, e.g., throw an exception or return an error code
        return;
    ...
    // Populate the matrix with DCT values
    for (uint8_t i = 0; i < _mfcc_size; i++) {
        for (uint8_t j = 0; j < _num_channels; j++) {
...
void arduinoMFCC::apply_mel_filter_bank(uint8_t  _num_channels, uint16_t  _frame_size,float *_frame,float ** _mel_filter_bank, float *_mfcc_coeffs) {
    // Serial.println("Applying Mel filter bank");
    for (uint8_t  i = 0; i < _num_channels; i++) {
        float output = 0.0f;
...
// Fonction publique pour appliquer la transformée de cosinus discrète (DCT) au signal audio
void arduinoMFCC::apply_dct(uint8_t  _mfcc_size, uint8_t  _num_channels,uint16_t  _frame_size,float **_mel_filter_bank,float *_mfcc_coeffs, float *_rmfcc_coeffs, float **_dct_matrix) {
    // Serial.println("Applying DCT");
    for (uint8_t  i = 0; i < _mfcc_size; i++) {
        float sum = 0.0;
        for (uint8_t  j = 0; j < _num_channels; j++) {
            sum += _mfcc_coeffs[j] *_dct_matrix[i][j];
        }
        _rmfcc_coeffs[i] =sum;
    }
}
